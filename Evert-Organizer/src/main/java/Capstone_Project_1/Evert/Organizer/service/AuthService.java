...
import Capstone_Project_1.Evert.Organizer.utils.JwtTokenUtil;
...
public class AuthService {
    ...
    public AuthResponse loginAttendee(LoginRequest request) {
        ...
        // Generate JWT token
        UserDetails userDetails = userDetailsService.loadUserByUsername(attendee.getUsername());
        String token = jwtTokenUtil.generateToken(userDetails, "attendee");

        AuthResponse response = new AuthResponse(true, "Login successful", "attendee", attendee.getUsername(), token);
        return response;
    }
    ...
    public AuthResponse loginOrganizer(LoginRequest request) {
        ...
        // Generate JWT token
        UserDetails userDetails = userDetailsService.loadUserByUsername(organizer.getUsername());
        String token = jwtTokenUtil.generateToken(userDetails, "organizer");

        AuthResponse response = new AuthResponse(true, "Login successful", "organizer", organizer.getUsername(), token);
        return response;
    }
