...
public class EventController {
    ...
    // Add these new endpoints to your EventController

    @GetMapping("/attendee/search")
    public ResponseEntity<?> searchUpcomingEventsForAttendee(
            @RequestParam String attendeeUsername,
            @RequestParam String searchTerm) {
        try {
            if (!SQLInjectionProtection.isSafe(attendeeUsername) || !SQLInjectionProtection.isSafe(searchTerm)) {
                return ResponseEntity.badRequest().body("Invalid input detected");
            }

            List<Event> events = eventService.searchUpcomingEventsForAttendee(attendeeUsername, searchTerm);
            return ResponseEntity.ok(events);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    @GetMapping("/attendee/search-booked")
    public ResponseEntity<?> searchBookedEventsForAttendee(
            @RequestParam String attendeeUsername,
            @RequestParam String searchTerm) {
        try {
            if (!SQLInjectionProtection.isSafe(attendeeUsername) || !SQLInjectionProtection.isSafe(searchTerm)) {
            ...
            List<Event> events = eventService.searchBookedEventsForAttendee(attendeeUsername, searchTerm);
            return ResponseEntity.ok(events);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error searching events: " + e.getMessage());
    ...
    @GetMapping("/attendee/search-deleted")
    public ResponseEntity<?> searchDeletedEventsForAttendee(
            @RequestParam String attendeeUsername,
            @RequestParam String searchTerm) {
        try {
            if (!SQLInjectionProtection.isSafe(attendeeUsername) || !SQLInjectionProtection.isSafe(searchTerm)) {
            ...
            List<Event> events = eventService.searchDeletedEventsForAttendee(attendeeUsername, searchTerm);
            return ResponseEntity.ok(events);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error searching events: " + e.getMessage());
